...
echo feats_fl: ${feats_fl?$0: feats_fl not specified}
echo infeats_tr: ${infeats_tr?$0: labels tr not specified}
echo infeats_cv: ${infeats_cv?$0: labels cv not specified}
...
$TRAIN_TOOL --cross-validate=true \
  --minibatch-size=$bunch_size --randomizer-size=$cache_size \
 ${feature_transform:+ --feature-transform=$feature_transform} \
 ${use_gpu_id:+ --use-gpu-id=$use_gpu_id} \
 $mlp_best "$infeats_cv" "$feats_fl" \
 2> $dir/log/prerun.log || exit 1;

acc=$(cat $dir/log/prerun.log | awk '/Mse/{ acc=$4; sub(/err.frm:/,"",acc); } END{print acc}')
...
for iter in $(seq -w $max_iters); do
  ...
  $TRAIN_TOOL \
   ...
   --minibatch-size=$bunch_size --randomizer-size=$cache_size --randomize=$randomize \
   ${feature_transform:+ --feature-transform=$feature_transform} \
   ${use_gpu_id:+ --use-gpu-id=$use_gpu_id} \
   ${seed:+ --randomizer-seed=$seed} \
   $mlp_best "$infeats_tr" "$feats_fl" $mlp_next \
   2> $dir/log/iter$iter.log || exit 1; 

  tr_acc=$(cat $dir/log/iter$iter.log | awk '/Mse/{ acc=$4; sub(/err.frm:/,"",acc); } END{print acc}')
  ...
  acc_new=$(cat $dir/log/iter$iter.log | awk '/Mse/{ acc=$4; sub(/err.frm:/,"",acc); } END{print acc}')
  echo -n "CROSSVAL ACCURACY $(printf "%.4f" $acc_new), "
  ...
  #stopping criterion
  if [[ "1" == "$halving" && "1" == "$(awk "BEGIN{print($acc > $acc_prev * (1.0 - $end_halving_inc))}")" ]]; then
    if [ \( "$min_iters" != "" \) -a \( "$min_iters" -gt "$iter" \) ]; then
      #if [ $min_iters -gt $iter ]; then
	echo we were supposed to finish, but we continue, min_iters : $min_iters
    else
	echo finished, too small improvement $(awk "BEGIN{print(($acc_prev - $acc)/$acc)}")
	break
    fi
  fi

  #start annealing when improvement is low
  if [ "1" == "$(awk "BEGIN{print($acc > $acc_prev * (1.0 - $start_halving_inc))}")" ]; then
    halving=1
  else
    halving=0
  fi
