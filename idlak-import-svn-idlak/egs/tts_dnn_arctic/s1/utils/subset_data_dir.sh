...
if [ -n "$spk" ]; then
  mkdir -p $destdir
  awk -v spk=$spk -v n=$numutt '($1 == spk){if (n <= 0) n = NF + n - 1; printf("%s ",$1); skip=1; while(n*(skip+1) <= NF-1) { skip++; }
  ...
  mkdir -p $destdir
  awk '{ n='$numutt'; if (n <= 0) n = NF + n - 1; printf("%s ",$1); skip=1; while(n*(skip+1) <= NF-1) { skip++; }
         for(x=2; x<=NF && x <= n*skip; x += skip) { printf("%s ", $x); } 
         printf("\n"); }' <$srcdir/spk2utt >$destdir/spk2utt
  utils/spk2utt_to_utt2spk.pl < $destdir/spk2utt > $destdir/utt2spk
  do_filtering; # bash function.
  ...
  if [ $numutt -gt `cat $srcdir/feats.scp | wc -l` ]; then
    echo "subset_data_dir.sh: cannot subset to more utterances than you originally had: $numutt gt `cat $srcdir/feats.scp | wc -l`"
    exit 1;
  fi 
