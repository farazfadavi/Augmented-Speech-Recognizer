// featbin/reverse-transform-feats.cc

// Copyright 2009-2014  Microsoft Corporation
//                      Johns Hopkins University (author: Daniel Povey)
//                      Idiap Research Institute (author: Blaise Potard)
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Apply reverse transform (e.g. LDA; HLDA; fMLLR/CMLLR; MLLT/STC)\n"
        "Linear transform if transform-num-cols == feature-dim, affine if\n"
        "transform-num-cols == feature-dim+1 (->append 1.0 to features)\n"
        "A key has to be provided for per-speaker or per-utterance transform\n"
        "Global if transform-rxfilename provided.\n"
        "Usage: reverse-transform-feats [options] (transform-rspecifier|transform-rxfilename) feats-rspecifier feats-wspecifier\n";
    ...
    Matrix<BaseFloat> rev_trans;
    Matrix<BaseFloat> featoff;
    ...
    for (;!feat_reader.Done(); feat_reader.Next()) {
      ...
      rev_trans.Resize(feat_dim, feat_dim);

      if (transform_cols == feat_dim) {
	rev_trans.CopyFromMat(trans);
	rev_trans.Invert();
        feat_out.AddMatMat(1.0, feat, kNoTrans, rev_trans, kTrans, 0.0);
	//feat_out.AddMatMat(1.0, feat, kNoTrans, trans, kTrans, 0.0);
        ...
        SubMatrix<BaseFloat> linear_part(trans, 0, feat_dim, 0, feat_dim);
	Vector<BaseFloat> offset(feat_dim);
	offset.CopyColFromMat(trans, feat_dim);
	featoff.Resize(feat.NumRows(), feat_dim);
	featoff.CopyFromMat(feat);
	featoff.AddVecToRows(-1.0, offset);
	rev_trans.CopyFromMat(linear_part);
	rev_trans.Invert();
        feat_out.AddMatMat(1.0, featoff, kNoTrans, rev_trans, kTrans, 0.0);
