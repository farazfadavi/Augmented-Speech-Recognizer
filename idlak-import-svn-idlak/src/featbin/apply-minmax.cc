// featbin/apply-minmax.cc

// Copyright 2013  IDIAP Research Institute
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Apply mean and variance normalization to ensure value are between 0.01 and 0.99\n"
        "Per-utterance by default, or per-speaker if utt2spk option provided\n"
        "Usage: apply-minmax [options] (minmax-stats-rspecifier|minmax-stats-rxfilename) feats-rspecifier feats-wspecifier\n";
    ...
    if (ClassifyRspecifier(cmvn_rspecifier_or_rxfilename, NULL, NULL)
      ...
      for (;!feat_reader.Done(); feat_reader.Next()) {
        ...
        ApplyMinmax(cmvn_stats, norm_vars, &feat);
    ...
    if (norm_vars) 
      KALDI_LOG << "Applied minmax transform to "
