// featbin/compute-minmax-stats.cc
...
#include "transform/minmax.h"

namespace kaldi {

bool AccMinmaxStatsWrapper(std::string utt,
			   const MatrixBase<BaseFloat> &feats,
			   RandomAccessBaseFloatVectorReader *weights_reader,
			   Matrix<double> *cmvn_stats) {
  if (!weights_reader->IsOpen()) {
    AccMinmaxStats(feats, NULL, cmvn_stats);
    return true;
  } else {
    ...
    AccMinmaxStats(feats, &weights, cmvn_stats);
    return true;
  }
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Compute statistics for having features values between 0.01 and 0.99\n"
        "If wspecifier provided: per-utterance by default, or per-speaker if\n"
        "spk2utt option provided; if wxfilename: global\n"
        "Usage: compute-minmax-stats  [options] feats-rspecifier (stats-wspecifier|stats-wxfilename)\n";
      ...
      if (spk2utt_rspecifier != "") {
        ...
        for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
          ...
          for (size_t i = 0; i < uttlist.size(); i++) {
            ...
            if (!is_init) {
              InitMinmaxStats(feats.NumCols(), &stats);
              is_init = true;
            }
            if (!AccMinmaxStatsWrapper(utt, feats, &weights_reader, &stats)) {
              num_err++;
            } else {
              num_done++;
            }
    ...
    KALDI_LOG << "Done accumulating Minmax stats for " << num_done
