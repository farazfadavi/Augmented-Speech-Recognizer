...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Converts alignments (containing transition-ids) to state-ids, zero-based.\n"
        "Usage:  ali-to-state  [options] <model> <alignments-rspecifier> <pdfs-wspecifier>\n"
        "e.g.: \n"
        " ali-to-state 1.mdl ark:1.ali ark, t:-\n";
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      for (size_t i = 0; i < alignment.size(); i++)
        alignment[i] = trans_model.TransitionIdToHmmState(alignment[i]);

      writer.Write(key, alignment);
      num_done++;
    }
    KALDI_LOG << "Converted " << num_done << " alignments to state sequences.";
