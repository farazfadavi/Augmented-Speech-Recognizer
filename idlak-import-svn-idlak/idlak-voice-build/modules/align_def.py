...
def main():
    ...
    os.system(com)
    com = 'ln -s %s %s' % (os.path.join(build_conf.kaldidir, 'egs', 'wsj', 's5', 'path.sh'),
                           os.path.join(build_conf.outdir, 'output', 'path.sh'))
    build_conf.logger.log('info', 'Linking wsj s5 path: %s' % (com))
    os.system(com)
    ...
    com = "cd %s/output; utils/prepare_lang.sh --num-nonsil-states %d data '<SIL>' data/lang data/lang" % (build_conf.outdir, NOSTATES)
