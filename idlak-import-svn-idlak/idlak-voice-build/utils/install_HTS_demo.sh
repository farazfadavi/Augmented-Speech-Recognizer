...
tar xfvz speech_tools-2.4-release.tar.gz
cd speech_tools
./configure
make
cd ..
...
./configure
make
#chmod +x examples/dumpfeats.sh
#ln -s `pwd`/examples/dumpfeats.sh `pwd`/examples/dumpfeats
cd ..
...
cd ..
    ...
    # NOTE: this version of STRAIGHT is compatible with this demo
